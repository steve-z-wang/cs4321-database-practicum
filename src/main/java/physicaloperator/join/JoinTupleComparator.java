...
public class JoinTupleComparator implements Comparator<Tuple> {
    ...
    public JoinTupleComparator(List<Column> leftSchema, List<OrderByElement> leftOrderByElements,
                               List<Column> rightSchema, List<OrderByElement> rightOrderByElements) {
        ...
        if (leftTupleIndexOrder.size() != rightTupleIndexOrder.size()) {
            throw new IllegalArgumentException("Left and right schema order by elements must be the same size");
        }
    }
     ...
     */
    private ArrayList<Integer> buildIndexOrder(List<Column> schema, List<OrderByElement> orderByElements) {
