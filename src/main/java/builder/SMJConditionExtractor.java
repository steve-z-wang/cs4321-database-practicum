 ...
 *
 * <p>We know the left and right operators and their columns we also have a list of condition /
 * expression that must has both column from left and right operator
 *
 * <p>so for each expression's left and right column, we first check if its from left or right
 * operator then we add the expression to the list of condition for that operator
 */
