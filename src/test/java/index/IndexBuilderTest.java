...
public class IndexBuilderTest {
  ...
  public static void setUpEnvironment() throws URISyntaxException, IOException {
    ...
    URI uri = Objects.requireNonNull(classLoader.getResource("unit_test_samples/index")).toURI();
  ...
  public void testIndexBuild1() throws IOException {
    ...
    IndexDefinition indexDefinition = IndexConfigManager.getInstance().getIndexConfigs().get(0);
    assertEquals("Boats", indexDefinition.getRelation());
  ...
  public void testIndexBuild2() throws IOException {
    ...
    // get the index definition
    IndexDefinition indexDefinition = IndexConfigManager.getInstance().getIndexConfigs().get(1);
    assertEquals("Sailors", indexDefinition.getRelation());
  ...
  private static void compareIndexTrees(String expectedIndexPath, String indexPath) throws IOException {
    // read the index file
  ...
  private static void compareIndexNodes(
    ...
    for (int j = 0; j < index.getNumOfKeys(); j++) {
      ...
      if (expectedindex.getKeyAtIndex(j) != index.getKeyAtIndex(j)) {
        System.out.println("Expected: " + expectedindex.getKeyAtIndex(j) + " Actual: " + index.getKeyAtIndex(j));
      }
